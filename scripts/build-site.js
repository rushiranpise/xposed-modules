...
function main() {
  ...
  const releasedCount = repos.filter((r) => r.release && r.release.tag).length;
...
<header>
  ...
  <p class="tagline">Verified Xposed &amp; LSPosed modules — the official LSPosed module repo plus modules confirmed by their code, kept up to date automatically.</p>
  <div class="stats">
    ...
    <span class="stat"><b id="stat-stars">0</b> total stars</span>
    <span class="stat"><b id="stat-releases">0</b> have releases</span>
...
<div class="toolbar">
  ...
  <select id="sort" aria-label="Sort order">
    ...
    <option value="updated">Sort: Recently committed</option>
...
<footer>
  ...
  each card shows the latest release and last commit, and new modules are added automatically, never duplicated.
</footer>
...
(function () {
  ...
  document.getElementById("stat-stars").textContent = fmt.format(DATA.total_stars);
  document.getElementById("stat-releases").textContent = fmtFull.format(DATA.released);
  ...
  function card(repo) {
    ...
    if (isOfficialOnly) {
      owner.textContent = "Official LSPosed module entry";
    } else {
      const parts = [];
    ...
    if (commitAt) updated.textContent = "commit " + timeAgo(commitAt);
    foot.append(updated);
