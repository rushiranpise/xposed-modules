...
<header>
  ...
  <p class="tagline">Every Xposed module repository we can find on GitHub — kept up to date automatically.</p>
  <div class="stats">
...
<footer>
  ...
  data lives in <code>data/repos.json</code> — new modules are added automatically, never duplicated.
</footer>
...
(function () {
  ...
  function card(repo) {
    ...
    owner.textContent = repo.owner ? "@" + repo.owner.login : "";
    ...
    if (repo.topics && repo.topics.length) {
      ...
      if (repo.topics.length > 4) {
        ...
        more.textContent = "+" + (repo.topics.length - 4);
        tags.append(more);
      }
      c.append(tags);
    }
    ...
    stars.textContent = "★ " + fmt.format(repo.stargazers_count || 0);
    foot.append(stars);
